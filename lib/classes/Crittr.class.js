...
class Crittr {
    ...
    constructor(options) {
        ...
        /**
         * We need to prevent parallel screenshot captures to avoid browser locking.
         * For more details, visit {@link https://github.com/puppeteer/puppeteer/issues/12712#issuecomment-2211081580}.
         * @see https://github.com/puppeteer/puppeteer/issues/12712#issuecomment-2211081580
         * @private
         */
    ...
    validateOptions() {
        ...
        if (!Array.isArray(this.options.css) && typeof this.options.css !== 'string' && this.options.css !== null) {
            errors.push({
                message: "'css' option type is not valid. Expected array or string, got " + typeof this.options.css,
            });
        }

        if (Array.isArray(this.options.css)) {
            if (this.options.css.length === 0) {
                errors.push(new Error("Array-type 'css' option must not be empty."));
            }

            const invalidCssItems = this.options.css.filter(css => typeof css !== 'string' && css.length > 0);

            if (invalidCssItems.length > 0) {
                errors.push(new Error("Array-type 'css' option must contain only non-empty string values."));
            }
        }
    ...
    /**
     * Tries to get the contents of the given css file(-s) or in case of css string returns the string
     *
     * @returns {Promise<any>}
     */
    async getCssContent() {
        if (Array.isArray(this.options.css)) {
            const cssContentArr = await Promise.all(this.options.css.filter(css => css.length > 0).map(css => this._getCssContent(css)));

            return cssContentArr.join('\n');
        }

        return this._getCssContent(this.options.css);
    }
    ...
    _getCssContent(css) {
        return new Promise(async (resolve, reject) => {
            if (typeof css === 'string') {
                ...
                if (css.endsWith('.css')) {
                    try {
                        cssString = await readFilePromise(css, 'utf8');
            ...
            return [...document.styleSheets]
                .map(styleSheet => {
                    try {
                        ...
                        log.log('Access to stylesheet %s is denied. Ignoring...', styleSheet.href);
                    }
                })
                .filter(Boolean)
                .join('\n');
        ...
        return new Promise(async (resolve, reject) => {
            ...
            queue
                ...
                .then(async () => {
                    ...
                    // Minify css
                    if (this.options.minifyCriticalCss) {
            ...
            if (hasError === false) {
                try {
                    ...
                    if (this.options.takeScreenshots === true) {
                        ...
                        await this.takeScreenshot(page, screenName);
                    }
    ...
    async takeScreenshot(page, screenName) {
        /**
         * We need to prevent parallel screenshot captures to avoid browser locking.
         * For more details, visit {@link https://github.com/puppeteer/puppeteer/issues/12712#issuecomment-2211081580}.
         * @see https://github.com/puppeteer/puppeteer/issues/12712#issuecomment-2211081580
        ...
        try {
            // The page must remain active while capturing the screenshot.
