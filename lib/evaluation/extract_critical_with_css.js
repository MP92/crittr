...
export default async ({ sourceAst, loadTimeout, keepSelectors, removeSelectors }) => {
    const resolvePseudoSelectorsRegex = pseudoSelectors =>
        new RegExp(
    ...
    return new Promise((resolve, reject) => {
        ...
        const pseudoFunctionalSelectors = ['is', 'not', 'where', 'has'];

        const PSEUDO_DEFAULT_REGEX = resolvePseudoSelectorsRegex(pseudoSelectors);

        const PSEUDO_EXCLUDED_REGEX = resolvePseudoSelectorsRegex(pseudoExcludes);

        const PSEUDO_FUNCTIONAL_REGEX = resolvePseudoSelectorsRegex(pseudoFunctionalSelectors);
         ...
         * @returns {boolean}
         */
        const isPurePseudo = selector =>
            selector.startsWith(':') && selector.match(PSEUDO_EXCLUDED_REGEX) === null && selector.match(PSEUDO_FUNCTIONAL_REGEX) === null;
        ...
        const _isElementAboveTheFold = (element) => {
            ...
            if (element.offsetParent === null && element.children.length > 0) {
                if (getComputedStyle(element).display === 'contents') {
                    return Array.from(element.children).some(child => isElementAboveTheFold(child));
                }
            }
        ...
        const isElementAboveTheFold = element => {
            ...
            const isAboveTheFold = _isElementAboveTheFold(element);
